$(document).ready(function () {
  ...
  function creador(tipo, contador) {
    switch (tipo) {
      case "normal":
        cat[contador] = group
          .create(100, 100, "catNormal")
          .setVelocity(Math.random() * 100 - 100, Math.random() * 100 - 100)
          .setScale(0.32);
        ...
        timedEventJob.delay =
          timedEventJob.delay - timedEventJob.delay * (contadorNormal * 0.01);
        break;
      case "fuerte":
        catStrong[contador] = group
          .create(100, 100, "catStrong")
          .setVelocity(Math.random() * 25 - 50, Math.random() * 25 - 50)
          .setScale(0.32);
        ...
        //Los eventos aparecen mas frecuentemente segun la cantidad de gatos.
        timedEventJob.delay =
          timedEventJob.delay - timedEventJob.delay * (contadorStrong * 0.01);
        break;
      case "rapido":
        catFast[contador] = group
          .create(100, 100, "catFast")
          .setVelocity(Math.random() * 100 - 100, Math.random() * 100 - 100)
          .setScale(0.32);
  ...
  function actualizarRecursos() {
    ...
    recursosSegundo = Math.trunc(
      ((contadorNormal * 2 + contadorStrong * 4) / (delay / 1000)) * 2
    );
  ...
  function mejoraRecursos(timedEvent) {
    if (recursos > costesMejora && mejoras < 10 && contadorNormal > 0) {
      ...
      timedEventResources.delay =
        timedEventResources.delay - timedEventResources.delay * 0.25;
    }
  }
      ...
      switch (typeJobEvent) {
        ...
        case 2:
          if (typeProbability == 3) {
            ...
            console.log(
              "Un gato estudio para mejorar pero no ha sido suficiente."
            );
          }
          break;
      }
  ...
  function eventos(typeCat, typeEvent) {
    switch (typeEvent) {
      case "recurso":
        var resourceEvent =
          cantidadEvento[Math.floor(Math.random() * cantidadEvento.length)];
        $(".textEventos").val(
          $(".textEventos").val() +
            "\n Un gato " +
            typeCat +
      ...
      case "mejora":
        $(".textEventos").val(
          $(".textEventos").val() +
            "\n Un gato " +
            typeCat +
            " encontró un libro de entrenamiento! Ha subido un nivel de mejora!"
        );
        mejoras++;
        $(".nivelMejora").html(mejoras);
        actualizarHtmlRecursos();
